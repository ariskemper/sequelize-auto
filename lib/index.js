...
AutoSequelize.prototype.build = function(callback) {
  ...
  function mapTable(table, _callback){
    self.queryInterface.describeTable(table).then(function(fields) {
      self.tables = removeFromArray(self.tables, table);
  ...
  function mapTables(err) {
    ...
    async.each(self.tables, mapTable, callback)
  }
  ...
  this.queryInterface.showAllTables().then(function (__tables) {
    ...
    self.tables = self.options.tables ? _.intersection(__tables, self.options.tables) : __tables;

    async.each(self.tables, mapForeignKeys, mapTables)
  }, function (err) {
    console.error(err);
    callback(err);
  });
  ...
  function mapForeignKeys(table, fn) {
    ...
    function assignColumnDetails(ref) {
      ...
      ref.isForeignKey = ! _.isEmpty(_.trim(ref.source_column)) && ! _.isEmpty(_.trim(ref.target_column));
      ref.isPrimaryKey = _.isFunction(self.dialect.isPrimaryKey) && self.dialect.isPrimaryKey(ref);
      ref.isSerialKey = _.isFunction(self.dialect.isSerialKey) && self.dialect.isSerialKey(ref);

      if (ref.isForeignKey || ref.isPrimaryKey) {
        self.foreignKeys[table] = self.foreignKeys[table] || {};
        self.foreignKeys[table][ref.source_column] = ref;
      }
    }
  }
}
...
AutoSequelize.prototype.run = function(callback) {
  ...
  function generateText(err) {
    ...
    var tableNames = _.keys(self.tables);
        ...
        _.each(fieldAttr, function(attr, x){
          ...
          if (attr === "foreignKey") {
            if (isSerialKey) {
             ...
             if (self.tables[table][field][attr] === true && (! self.tables[table][field].hasOwnProperty('foreignKey') || (self.tables[table][field].hasOwnProperty('foreignKey') && !!self.tables[table][field].foreignKey.isPrimaryKey)))
...
AutoSequelize.prototype.write = function(attributes, callback) {
  ...
  function createFile(table, _callback){
    var fileName = table.charAt(0).toUpperCase() + string.slice(1);
